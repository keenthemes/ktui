...
export function createSortHandler<T = KTDataTableDataInterface>(
  ...
  getState: () => { sortField: keyof T | number; sortOrder: KTDataTableSortOrderInterface },
  ...
  function compareValues(a: unknown, b: unknown, sortOrder: KTDataTableSortOrderInterface): number {
  ...
  function sortData(data: T[], sortField: keyof T | number, sortOrder: KTDataTableSortOrderInterface): T[] {
  ...
  function toggleSortOrder(currentField: keyof T | number, currentOrder: KTDataTableSortOrderInterface, newField: keyof T | number): KTDataTableSortOrderInterface {
  ...
  function setSortIcon(sortField: keyof T, sortOrder: KTDataTableSortOrderInterface): void {
    ...
    if (th) {
      const sortElement = th.querySelector(`.${config.sort?.classes?.base}`) as HTMLElement;
      if (sortElement) {
        sortElement.className = `${config.sort?.classes?.base} ${sortClass}`.trim();
      }
    }
  }
  ...
  function initSort(): void {
    ...
    headers.forEach((header) => {
      ...
      header.addEventListener('click', () => {
        ...
        const sortOrder = toggleSortOrder(state.sortField, state.sortOrder, sortField);
