...
export class KTDatepickerKeyboard {
  ...
  private _setupEventListeners(): void {
    ...
    this._eventManager.addEventListener(KTDatepickerEventName.OPEN, () => this._activateKeyboardNavigation());
    this._eventManager.addEventListener(KTDatepickerEventName.CLOSE, () => this._deactivateKeyboardNavigation());

    // Listen for custom keyboard-open event
    this._eventManager.addEventListener(KTDatepickerEventName.KEYBOARD_OPEN, () => {
    ...
    const displayElement = this._element.querySelector('[data-kt-datepicker-display]');
    if (displayElement) {
      displayElement.addEventListener('keydown', (e: KeyboardEvent) => {
        if (e.key === 'Enter') {
          e.preventDefault();
          e.stopPropagation();
  ...
  private _handleDaysViewKeyNavigation(e: KeyboardEvent): void {
    ...
    const firstDayOffset = (firstDayOfMonth - config.locales[config.locale].firstDayOfWeek + 7) % 7;
    ...
    switch (e.key) {
      ...
      case 'ArrowDown':
        ...
        if (this._focusedDay && this._focusedDay > lastRowStart) {
          ...
          this._focusedDay = dayPosition + 1 - ((dayPosition + firstDayOffset) % 7);
      ...
      case ' ':
        ...
        if (this._focusedDay) {
          ...
          if (!config.range ||
              (state.selectedDateRange && state.selectedDateRange.startDate && state.selectedDateRange.endDate)) {
            this._stateManager.setOpen(false);
          }
        }
        break;
  ...
  private _focusDay(): void {
    ...
    if (!dropdown) {
      ...
      dropdown = this._element.querySelector('.multiple-months') ||
    ...
    let dayButton = dropdown.querySelector(`button[data-date="${currentDay}"]:not(.text-gray-400)`);
