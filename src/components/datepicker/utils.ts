 ...
 */
export function formatDate(date: Date, format: string, config: KTDatepickerConfigInterface): string {
 ...
 */
export function parseDate(dateStr: string, format: string, config: KTDatepickerConfigInterface): Date | null {
  ...
  let formatRegex = format
    .replace(/(\w)(\1*)/g, (_, p1, p2) => {
      ...
      switch (p1) {
        case 'y': token = length > 2 ? 'yyyy' : 'yy'; break;
        case 'M': token = ['M', 'MM', 'MMM', 'MMMM'][Math.min(length - 1, 3)]; break;
        case 'd': token = length > 1 ? 'dd' : 'd'; break;
        case 'E': token = length > 3 ? 'EEEE' : length > 1 ? 'EEE' : 'E'; break;
        case 'h': case 'H': token = length > 1 ? p1 + p1 : p1; break;
        case 'm': token = length > 1 ? 'mm' : 'm'; break;
        case 's': token = length > 1 ? 'ss' : 's'; break;
        case 'a': case 'A': token = p1; break;
        default: token = p1.repeat(length);
      }
  ...
  if (formatParts.MM || formatParts.M) {
    ...
    const locale = getLocaleConfig(config);
    const monthIndex = locale.monthNames.findIndex(m => m.toLowerCase() === monthName.toLowerCase());
    if (monthIndex === -1) {
      const shortMonthIndex = locale.monthNamesShort.findIndex(m => m.toLowerCase() === monthName.toLowerCase());
...
function parseNaturalLanguageDate(input: string): Date | null {
  ...
  switch (normalized) {
    ...
    default: {
      ...
      const relativeMatch = normalized.match(/^(next|last|this)\s+(day|week|month|year)$/);
      if (relativeMatch) {
        ...
        switch (unit) {
          case 'day':
            result.setDate(result.getDate() + (direction === 'next' ? 1 : direction === 'last' ? -1 : 0));
            break;

          case 'week':
            result.setDate(result.getDate() + (direction === 'next' ? 7 : direction === 'last' ? -7 : 0));
            break;

          case 'month':
            result.setMonth(result.getMonth() + (direction === 'next' ? 1 : direction === 'last' ? -1 : 0));
            break;

          case 'year':
            result.setFullYear(result.getFullYear() + (direction === 'next' ? 1 : direction === 'last' ? -1 : 0));
            break;
        }
 ...
 */
export function getLocaleConfig(config: KTDatepickerConfigInterface): LocaleConfigInterface {
 ...
 */
export function isDateDisabled(date: Date, config: KTDatepickerConfigInterface): boolean {
  if (!date || !(date instanceof Date) || isNaN(date.getTime())) {
    return true;
  }
 ...
 */
export function generateCalendarMonth(year: number, month: number, config: KTDatepickerConfigInterface): Date[][] {
