...
class FocusManager {
  ...
  private _focusableSelector: string = 'button:not([disabled]), [tabindex]:not([tabindex="-1"])';
  ...
  public getVisibleOptions(): HTMLElement[] {
    return Array.from(this._element.querySelectorAll(this._focusableSelector))
      .filter(el => {
  ...
  public focusPrevious(): void {
    ...
    if (currentFocused) {
      const currentIndex = options.indexOf(currentFocused as HTMLElement);
      prevIndex = currentIndex >= 0 ? (currentIndex - 1 + options.length) % options.length : prevIndex;
    }
...
class EventManager {
   ...
   */
  public addListener(element: HTMLElement, eventType: string, handler: Function): void {
...
class FocusTrap {
  ...
  private _update(): void {
    ...
    this._focusableElements = Array.from(focusableElements)
...
export class KTDatepickerDropdown extends KTComponent {
  ...
  public updatePosition(): void {
    ...
    const displayElement = this._element.querySelector('[data-kt-datepicker-display]') as HTMLElement;
  ...
  private _focusFirstInteractiveElement(): void {
    ...
    const currentMonthDay = this._dropdownElement.querySelector('button[data-date]:not(.text-gray-400)') as HTMLElement;
    if (currentMonthDay) {
