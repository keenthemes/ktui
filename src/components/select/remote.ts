...
export class KTSelectRemote {
    ...
    private _processData(data: any): KTSelectOptionData[] {
        try {
            ...
            if (this._config.apiDataProperty && data[this._config.apiDataProperty]) {
                if (this._config.debug) console.log(`Extracting data from property: ${this._config.apiDataProperty}`);

                // If pagination metadata is available, extract it
                if (this._config.pagination) {
                    if (data.total_pages) {
                        this._totalPages = data.total_pages;
                        if (this._config.debug) console.log(`Total pages found: ${this._totalPages}`);
                    }
                    if (data.total) {
                        this._totalPages = Math.ceil(data.total / (this._config.paginationLimit || 10));
                        if (this._config.debug) console.log(`Calculated total pages: ${this._totalPages} from total: ${data.total}`);
                    }
                }
            ...
            if (this._config.debug) console.log(`Mapping ${processedData.length} items to KTSelectOptionData format`);
            ...
            const mappedData = processedData.map((item: any): KTSelectOptionData => {
                ...
                if (this._config.dataValueField && this._config.dataValueField.includes('.')) {
                    ...
                    if (nestedValue !== null && nestedValue !== undefined) {
                        ...
                        if (this._config.debug) console.log(`Data path verification for [${this._config.dataValueField}]: Expected: ${expectedValue}, Got: ${mappedItem.id}`);

                        if (mappedItem.id !== expectedValue && expectedValue) {
                            console.warn(`Value mismatch! Path: ${this._config.dataValueField}, Expected: ${expectedValue}, Got: ${mappedItem.id}`);
                        }
                    }
                }

                if (this._config.debug) console.log(`Mapped item: ${JSON.stringify(mappedItem)}`);
                return mappedItem;
            });

            if (this._config.debug) console.log(`Returned ${mappedData.length} mapped items`);
            return mappedData;
        } catch (error) {
    ...
    private _mapItemToOption(item: any): KTSelectOptionData {
        ...
        if (this._config.debug) console.log(`Mapping fields: value=${valueField}, label=${labelField}, description=${descriptionField}, icon=${iconField}`);
        if (this._config.debug) console.log('Item data:', JSON.stringify(item).substring(0, 200) + '...'); // Trimmed for readability
        ...
        const getValue = (obj: any, path: string): any => {
            ...
            try {
                ...
                if (this._config.debug) console.log(`Extracted [${path}] => ${result !== null && result !== undefined ?
        ...
        if (id === null || id === undefined) {
            ...
            if (item.id && typeof item.id === 'object' && 'value' in item.id && item.id.value) {
                id = String(item.id.value);
                if (this._config.debug) console.log(`Using id.value as fallback: ${id}`);
            } else if (item.id) {
                id = String(item.id);
                if (this._config.debug) console.log(`Using direct item.id as fallback: ${id}`);
            } else {
                // If no ID found at all, use the title instead (will be extracted below)
                if (this._config.debug) console.log(`No ID found, will use title as fallback`);
                id = null;
            }
        } else if (typeof id === 'object') {
            // If ID is an object, log the issue and set to null to use title fallback
            console.warn(`ID for path ${valueField} is an object, will use title fallback instead`);
            id = null;
        } else {
        ...
        if (this._config.debug) console.log(`Title/label field [${labelField}]:`, title);
        ...
        if (!title) {
            ...
            if (this._config.debug) console.log('After fallback checks, title:', title);
        }
        ...
        if (description === null || description === undefined ||
            String(description) === 'null' || String(description) === 'undefined') {
            description = null;
        } else {
            description = String(description);
        }
        if (this._config.debug) console.log(`Description field [${descriptionField}]:`, description);

        // Try to get an icon - make sure we don't pass null, undefined, or "null" string values
        let icon = getValue(item, iconField);
        if (icon === null || icon === undefined ||
            String(icon) === 'null' || String(icon) === 'undefined') {
            icon = null;
        } else {
            icon = String(icon);
        }
        ...
        if (id === null || id === '') {
            id = title;
            if (this._config.debug) console.log(`Using title as fallback for ID: ${id}`);
        }
        ...
        if (this._config.debug) console.log('Final mapped item:', JSON.stringify(result));
        return result;
    }
