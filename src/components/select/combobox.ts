...
export class KTSelectCombobox {
  ...
  private _attachEventListeners(): void {
    ...
    if (this._config.debug) console.log('Combobox event listeners attached to:', this._searchInputElement);
  }
  ...
  private _handleComboboxKeyNav(event: KeyboardEvent): void {
    ...
    if (!(this._select as any)._dropdownIsOpen &&
      ...
      if (this._config.debug) console.log('Opening dropdown from keyboard in combobox');
      this._select.openDropdown();
      event.preventDefault();
  ...
  public updateSelectedValue(selectedText: string): void {
    if (this._searchInputElement) {
      ...
      if (this._config.debug) console.log('Combobox value updated to:', cleanText);
    }
  }
  ...
  public resetInputValueToSelection(): void {
    ...
    if (selectedOptions.length > 0) {
      ...
      if (selectedOption) {
        ...
        const titleElement = selectedOption.querySelector('[data-kt-option-title]');
        let selectedText = '';
