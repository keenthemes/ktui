...
export class KTSelectDropdown extends KTComponent {
  ...
  constructor(
    ...
    this._focusManager = new FocusManager(dropdownElement, '[data-kt-select-option]', config);
  ...
  private _setupEventListeners(): void {
    ...
    this._eventManager.addListener(document as unknown as HTMLElement, 'click', this._handleOutsideClick.bind(this));
  }
   ...
   */
  public toggle(): void {
    if (this._config.debug) console.log('KTSelectDropdown.toggle called - isOpen:', this._isOpen);

    if (this._isTransitioning) {
      if (this._config.debug) console.log('KTSelectDropdown.toggle - ignoring during transition');
      return;
    }
  ...
  public open(): void {
    ...
    KTDom.transitionEnd(this._dropdownElement, () => {
      ...
      if (this._config.enableSearch) {
        const searchInput = this._dropdownElement.querySelector('input[type="search"]');
        if (searchInput) {
   ...
   */
  public close(): void {
    if (this._config.debug) console.log('KTSelectDropdown.close called - isOpen:', this._isOpen, 'isTransitioning:', this._isTransitioning);

    if (!this._isOpen || this._isTransitioning) {
      if (this._config.debug) console.log('KTSelectDropdown.close - early return: dropdown not open or is transitioning');
      return;
    }
    ...
    if (beforeHideEvent.defaultPrevented) {
      if (this._config.debug) console.log('KTSelectDropdown.close - canceling due to defaultPrevented on beforeHideEvent');
      return;
    }

    if (this._config.debug) console.log('KTSelectDropdown.close - starting transition');
    ...
    const fallbackTimer = setTimeout(() => {
      if (!transitionComplete) {
        if (this._config.debug) console.log('KTSelectDropdown.close - fallback timer triggered');
        completeTransition();
      }
    ...
    const completeTransition = () => {
      ...
      if (this._config.debug) console.log('KTSelectDropdown.close - transition ended');
      ...
      if (this._config.debug) console.log('KTSelectDropdown.close - complete, events fired');
    };
